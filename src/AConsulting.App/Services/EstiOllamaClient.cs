...
/// <summary>
/// Desktop-only ESTI → local Ollama. Mirror of AStudio S4 — never the hub/VPS.
...
public sealed class EstiOllamaClient : IDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        var system = """
            You are ESTI, the local practice assistant inside AConsulting (AORMS).
            Answer for an engineering consultancy principal under time pressure.
            In ≤120 words cover: where we are · what matters · what to do now · what next.
            Use Indian practice language when relevant (GST, COA, RACI, go/no-go). Never invent money or quantities — those come from bbs_engine / AQC.
            Do not mention being an AI model unless asked. Desktop-only: no cloud hub claims.
            """;

        var user = string.IsNullOrWhiteSpace(practiceContext)
            ? userText.Trim()
