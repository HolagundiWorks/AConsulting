...
public sealed partial class MainWindow : Window
{
    ...
    async void Activate_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            LogText.Text = "Activatingâ€¦";
            var grant = await _bridge.ActivateAsync(key);
            RefreshStatus($"Activate OK Â· syncToken length={grant.SyncToken?.Length ?? 0}");
        }
        catch (Exception ex)
        {
    ...
    {
        var id = _bridge.EnqueueMeta("phaseProgress", "aconsulting-shell-1", new Dictionary<string, object?>
        {
            ["projectId"] = "00000000-0000-0000-0000-000000000001",
            ["phaseId"] = "aconsulting-shell-1",
            ["pctComplete"] = 10,
            ["status"] = "IN_PROGRESS",
            ["source"] = "AConsulting.App",
        });
